...
function openPaymentModal(rentalId) {
    ...
    document.getElementById('paySummaryRef').textContent = `Rental #${rentalId}`;
    document.getElementById('paySummaryStudent').textContent = r ? (r.studentName || '—') : '—';
    document.getElementById('paySummaryAmount').textContent = `RM ${dep}`;
    document.getElementById('payCashRef').textContent = `#${rentalId}`;
    ...
    const cardExp = document.getElementById('payCardExpiry');
...
function validateGatewayDetails() {
    ...
    if (gatewaySelectedMethod === 'ONLINE_BANKING') {
        const bank = document.getElementById('payBankCode').value;
        if (!bank) {
...
async function submitGatewayPayment() {
    ...
    try {
        const createRes = await authFetch('/api/payments', {
            method: 'POST',
            body: JSON.stringify({ rentalId: Number(rentalId), method: gatewaySelectedMethod })
        });
